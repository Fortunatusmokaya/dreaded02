...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         if (!text) return client.reply(m.chat, Func.example(isPrefix, command, 'alan walker alone'), m)
         ...
         if (chSize.oversize) return client.reply(m.chat, `💀 File size (${json.data.size}) exceeds the maximum limit, download it by yourself!`, m)
