      ...
      try {
         ...
         if (command == 'gc') {
            ...
            if (/1D|7D|30D/.test(dial)) {
               ...
               return client.reply(m.chat, Func.texted('bold', `🗿 Owner has Successfully set bot to stay forever in ${groupName} group.`), m)
            } else if (dial == 2) {
               ...
               await client.reply(jid, `👋 Goodbye!, the bot will leave the group for some reason.`, null, {
                  mentions: groupMetadata.participants.map(v => v.id)
               }).then(() => {
                  client.groupLeave(jid).then(() => {
                     global.db.groups[jid].expired = 0
                     global.db.groups[jid].stay = false
                     return client.reply(m.chat, Func.texted('bold', `Bot has Successfully left from ${groupName} group.`), m)
                  })
               })
            } else if (dial == 4) {
               global.db.groups[jid].mute = true
               client.reply(m.chat, Func.texted('bold', `Owner Bot has successfully muted the bot in ${groupName} group.`), m)
            } else if (dial == 5) {
               global.db.groups[jid].mute = false
               client.reply(m.chat, Func.texted('bold', `Owner has successfully unmuted the bot in ${groupName} group.`), m)
            } else if (dial == 6) {
               if (!admin) return client.reply(m.chat, Func.texted('bold', `⚠️ Can't close ${groupName} group link because the bot is not an admin.`), m)
               client.groupSettingUpdate(jid, 'announcement').then(() => {
                  client.reply(jid, Func.texted('bold', ` Group has been closed remotely by Owner bot.`)).then(() => {
                     client.reply(m.chat, Func.texted('bold', ` Successfully closed ${groupName} group.`), m)
                  })
               })
            } else if (dial == 7) {
               if (!admin) return client.reply(m.chat, Func.texted('bold', `⚠️ Can't open ${groupName} group link because the bot is not an admin.`), m)
               client.groupSettingUpdate(jid, 'not_announcement').then(() => {
                  client.reply(jid, Func.texted('bold', ` Group has been opened remotely by owner bot.`)).then(() => {
                     client.reply(m.chat, Func.texted('bold', `Successfully opened ${groupName} group.`), m)
                  })
               })
            } else if (dial == 8) {
