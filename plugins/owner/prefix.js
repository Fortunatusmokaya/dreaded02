exports.run = {
   ...
   async: async (m, {
      ...
      if (command == 'prefix') {
         ...
         if (evaluate_chars.includes(args[0])) return client.reply(m.chat, Func.texted('bold', ` Cannot Change prefix ${args[0]} karena akan terjadi error.`), m)
         ...
         if (!args || !args[0]) return client.reply(m.chat, Func.example(isPrefix, command, '#'), m)
         if (args[0].length > 1) return client.reply(m.chat, Func.texted('bold', `Enter only 1 prefix.`), m)
         if (system.prefix.lenght < 2) return client.reply(m.chat, Func.texted('bold', `Can't remove more prefix.`), m)
