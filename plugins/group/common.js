...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (arr.length == 0) return client.reply(m.chat, Func.texted('bold', ` No common groups with bots.`), m)
         ...
         client.sendList(m.chat, '', `Bot and @${user.replace(/@.+/,'')} are in same *${arr.length}* groups. 🗿`, '', 'Tap Here!', [{
            rows
         }], m)
      }
   },
