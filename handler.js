   ...
   try {
      ...
      if ((body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) || body && commands.includes((body.split` ` [0]).toLowerCase())) {
         ...
         if (!commands.includes(command) && matcher.length > 0 && !setting.self) {
            if (!m.isGroup || (m.isGroup && !groupSet.mute)) return client.reply(m.chat, `Did you mean :\n\n${matcher.map(v => '➠ *' + (isPrefix ? isPrefix : '') + v.string + '* (' + v.accuracy + '%)').join('\n')}`, m)
         }
         ...
         for (let name in is_commands) {
            ...
            if (setting.pluginDisable.includes(name)) return client.reply(m.chat, Func.texted('bold', `Command disabled by Owner.`), m)
            if (!m.isGroup && !['owner', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && new Date() * 1 - chats.lastchat < global.timer) continue
            if (!m.isGroup && !['owner', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `Currently bot is in group mode only!`, m, {
               largeThumb: true,
            ...
            if (cmd.restrict && !isOwner && text && new RegExp('\\b' + global.db.setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, ` You violated the *Terms & Conditions* of using this bot by using blacklisted keywords`, m).then(() => {
                  users.banned = true
            ...
            if (cmd.limit && users.limit < 1) {
               return client.reply(m.chat, `Your bot usage has reached the limit and will be reset at 00.00`, m).then(() => users.premium = false)
               continue
            }
