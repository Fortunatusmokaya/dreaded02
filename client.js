...
const connect = async () => {
   ...
   global.client = Socket({
      ...
      msgRetryCounterMap,
      browser: ['dreaded/ fortunatus', 'safari', '1.0.0'],
      auth: state,
   ...
   client.ev.on('group-participants.update', async (room) => {
      ...
      if (room.action == 'add') {
         if (groupSet.localonly) {
            if (typeof global.db.users[member] != 'undefined' && !global.db.users[member].whitelist && !member.startsWith('254') || !member.startsWith('254')) {
               client.reply(room.id, Func.texted('bold', `Sorry @${member.split`@`[0]}, this group is only for Kenyan people. Good Bye...`))
